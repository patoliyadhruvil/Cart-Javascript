...
const handleClick = (id) =>{
    ...
    help.innerHTML = myProduct.length;
    ...
    let pro  = products.filter((product)=>{
        return product.id == id
    })
    myProduct.push(pro[0]);
    help.innerHTML = myProduct.length;
    console.log("single" , myProduct);
...
let product;
let row = document.getElementById('myProduct');
...
products.forEach((product)=>{
    ...
    row.innerHTML += `<div class="col-3">
        <div class="box shadow-lg p-3 mb-5 bg-body rounded border border-primary">
            ...
            <p class="text-info">${product.discountPercentage}</p>
            <p class="text-warning">${product.rating}</p>
            <button class="btn btn-warning" onclick="handleClick(${product.id})">Add to Cart</button>
            <div>
        </div>
    </div>`
})  